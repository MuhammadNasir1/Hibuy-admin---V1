    ...
    <div class="w-full pt-10 min-h-[86vh]   rounded-lg custom-shadow">
        ...
        <div class="mt-5 mb-2 px-5">
            <ul class="flex flex-wrap text-sm font-medium text-center text-gray-500 dark:text-gray-400">
                <li class="me-2">
                    <a href="#" id="allTab"
                        class="filter-tab border inline-block px-4 py-1 text-white bg-primary rounded-3xl active"
                        aria-current="page">All</a>
                </li>
                <li class="me-2">
                    <a href="#" id="boostedTab"
                        class="filter-tab border inline-block px-4 py-1 rounded-3xl hover:text-gray-900 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-white">Boosted</a>
                </li>
            </ul>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="dataTable " id="product-table">
                <thead class="w-full font-semibold text-white bg-primary">
                    <tr class="w-full border-b whitespace-nowrap">
                        @foreach ($headers as $header)
                            <th scope="col"
                                class="px-4 py-3 text-left text-xs font-medium text-white bg-primary uppercase tracking-wider">
                                {{ $header }}
                            </th>
                        @endforeach
                    </tr>
                </thead>
                <tbody id="productListContainer" class="border-b border-gray-500">
                    @php $displayCounter = 1; @endphp
                    @foreach ($products as $product)
                        @if (!empty($product->product_category))
                            <tr>
                                <td class="px-4 py-2">{{ $displayCounter++ }}</td>
                                <td class="px-4 py-2">
                                    <img class="rounded-full w-11 h-11"
                                        src="{{ $product->first_image ? asset($product->first_image) : asset('asset/Ellipse 2.png') }}"
                                        alt="Product Image">
                                </td>
                                <td class="px-4 py-2 capitalize">{{ $product->product_name }}</td>
                                <td class="px-4 py-2 capitalize">{{ $product->product_category }}</td>
                                <td class="px-4 py-2">RS {{ $product->product_discounted_price }}</td>
                                <td class="px-4 py-2">{{ \Carbon\Carbon::parse($product->created_at)->format('M d, Y') }}
                                </td>
                                <td class="px-4 py-2 capitalize">{{ $product->user_name }}</td>
                                <td class="px-4 py-2">
                                    <span
                                        class="whitespace-nowrap px-2 py-1 text-xs font-semibold text-white rounded {{ $product->is_boosted == 0 ? 'bg-red-500' : 'bg-green-500' }}">
                                        {{ $product->is_boosted == 0 ? 'Not Boosted' : 'Boosted' }}
                                    </span>
                                </td>
                                <td class="px-4 py-2">
                                    <span
                                        class="whitespace-nowrap px-2 py-1 text-xs font-semibold text-white rounded {{ $product->product_status == 0 ? 'bg-red-500' : 'bg-green-500' }}">
                                ...
                                <td>
                                    ...
                                    <span class="flex gap-4 items-center">
                                        ...
                                        @if (session('user_details.user_role') !== 'admin')
                                            ...
                                            <button delurl="/delete-product/{{ $product->product_id }}"
                                                class="deleteDataBtn">
                                                <svg width='36' height='36' viewBox='0 0 36 36' fill='none'
                                                    xmlns='http://www.w3.org/2000/svg'>
                                                    <circle opacity='0.1' cx='18' cy='18' r='18'
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            function loadProducts(boosted = '') {
                let url = "/products";
                if (boosted === '1') url += "?boosted=1";

                // Show loading indicator in tbody
                $("#productListContainer").html(
                    '<tr><td colspan="10" class="text-center py-4">Loading...</td></tr>');

                $.get(url, function(responseHtml) {
                    // 1. Extract the new tbody rows from the fetched HTML
                    const newRows = $(responseHtml).find("#productListContainer").html();

                    // 2. Destroy the existing DataTable (if already initialized)
                    if ($.fn.DataTable.isDataTable('#product-table')) {
                        $('#product-table').DataTable().clear().destroy();
                    }

                    // 3. Replace only the table body with new rows
                    $("#productListContainer").html(newRows);

                    // 4. Reinitialize DataTable on the updated table
                    $('#product-table').DataTable({
                        paging: true,
                        searching: true,
                        ordering: true,
                    ...
                    // 5. Reattach event handlers (View, Edit, Delete buttons, etc.)
            ...
            $(".filter-tab").on("click", function(e) {
                ...
                $(".filter-tab").removeClass("text-white bg-primary active")
                    .addClass("hover:text-gray-900 hover:bg-gray-100");
                $(this).addClass("text-white bg-primary active")
                    .removeClass("hover:text-gray-900 hover:bg-gray-100");

                const isBoosted = this.id === "boostedTab" ? '1' : '';
                loadProducts(isBoosted);
            });

            // View button click handler (using event delegation)
            $(document).on("click", ".viewModalBtn", function() {
                let viewproducturl = $(this).attr("viewproducturl");
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $("#product_status").val(response.product.product_status.toString())
                                .change();

                            // Group parent variations
                            ...
                            if (images && images.length > 0) {
                                ...
                                $("#sub-image-3").attr("src", images[3] || defaultImage);
                                $("#sub-image-4").attr("src", images[4] || defaultImage);
                            }
                        } else {
            ...
            $("#statusForm").submit(function(event) {
                ...
                let productStatus = $("#product_status").val();
                let productId = $("#edit_status_id").val();
