...
@section('nav-title', 'Help Center')
@section('content')
    <div class="w-full pt-10 min-h-[86vh]   rounded-lg custom-shadow">
        <div class="flex justify-between items-center px-5">
            <h2 class="text-2xl font-medium  ">Help Center FAQs</h1>
                <div>
                    @if (session('user_details.user_role') == 'admin' || canMenuAction('help-center', 'add'))
                        <button id="addModalBtn" data-modal-target="add-faq-category-modal"
                            data-modal-toggle="add-faq-category-modal"
                            class="px-3 py-2 font-semibold text-white rounded-full bg-primary">
                            Add Category
                        </button>
                        <button id="addFaqModalBtn" type="button" data-modal-target="add-faq-modal"
        ...
        <x-table :headers="['ID', 'Category Icon', 'Category', 'Question', 'Answer', 'Status', 'Action']">
            <x-slot name="tablebody">
                @foreach ($faqs as $faq)
                    <tr>
                        ...
                        <td>
                            @if ($faq->category && $faq->category->image)
                                <img class="rounded-full w-11 h-11" src="{{ asset($faq->category->image) }}"
                                    alt="Category Icon">
                            @else
                        ...
                        <td class="capitalize">{{ $faq->question }}</td>
                        <td>{{ $faq->answer }}</td>
                        ...
                        <td>
                            <span class="flex gap-4">
                                @if (session('user_details.user_role') == 'admin' || canMenuAction('help-center', 'edit'))
                                    <button editurl="{{ route('faq.edit', $faq->faq_id) }}" class="updateDataBtn">
                                ...
                                @endif
                                @if (session('user_details.user_role') == 'admin' || canMenuAction('help-center', 'delete'))
                                    <button delurl="{{ route('faq.delete', $faq->faq_id) }}" class="deleteDataBtn">
                                        <svg width='36' height='36' viewBox='0 0 36 36' fill='none'
                                            xmlns='http://www.w3.org/2000/svg'>
